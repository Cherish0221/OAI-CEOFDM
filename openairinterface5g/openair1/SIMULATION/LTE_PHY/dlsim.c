	  ...
	  //PMI_FEEDBACK:
		...
		if (n_frames==1)
          ...
          /*
            ...
            OFDM_SYMBOL_SIZE_CLOMPLEX_SAMPLES);
...
//加的
		FILE *fp;
		int n;		
		fp = fopen("a.txt", "w");//打开文件以便写入数据		
		for (i = 0; i <nsymb*eNB->frame_parms.ofdm_symbol_size; i++) //将a数组中的整数写入fp指向的a.txt文件
{  
			fprintf(fp, "%d\n", eNB->common_vars.txdataF[eNB_id][0][i]);
		}
		fclose(fp);               //写入完毕，关闭文件
            write_output("txsigF1.m","txsF1", &eNB->common_vars.txdataF[eNB_id][0][subframe*nsymb*eNB->frame_parms.ofdm_symbol_size],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);

/*-----------------------------抓取数据符号：5M--300个;10M--600----------- ----------------*/
...
*/
	...
	fp = fopen("1.txt", "w");//打开文件以便写入数据
	for (i = 0; i <512 * 14; i++) {  //将a数组中的整数写入fp指向的c:\a.txt文件
		fprintf(fp, "%d\n", eNB->common_vars.txdataF[eNB_id][0][i]);
	}
	fclose(fp);
	...
	for (k = 0; k < 14; k++)
	{
		for (j = 512 * k + 1; j<512 * k + 151; j++){ a[0][0][j - 1 - 212 * k] = eNB->common_vars.txdataF[eNB_id][0][j]; }
		for (j = 512 * k + 362; j<512 * k + 512; j++){ a[0][0][j - 212 * (k + 1)] = eNB->common_vars.txdataF[eNB_id][0][j]; }
	//	printf("初始第%d个数据符号： \n", k + 1);
	...
	/*2.txt 检测提取300*14个符号点是否正确______修改完成*/

	fp = fopen("2.txt", "w");//打开文件以便写入数据
	for (i = 0; i <300 * 14; i++)
	{  //将a数组中的整数写入fp指向的2.txt文件
		fprintf(fp, "%d\n", a[0][0][i]);
	}
	fclose(fp);
	...
	{
		printf("第%d个OFDM共轭：  \n",k+1);
		aaaaa(&a[0][0][300*k], 300, 512, &eNB->common_vars.txdataF[eNB_id][0][1024*k]);
//aaaaa(&a[0][0][0], 300, 512, &txdataf[0][0][0]);
	}
	

	//printf("共轭映射后序列： \n");
	for (j = 0; j<512 * 2*14; j++){
		//printf("%d \n", eNB->common_vars.txdataF[eNB_id][0][j]);
	}

write_output("3.m","3", &eNB->common_vars.txdataF[eNB_id][0][0],1024*14,1,1);
	/*3.txt一个符号的共轭映射是否正确

	fp = fopen("3.m", "w");//打开文件以便写入数据

	for (i = 0; i <1024*14; i++) {  //将a数组中的整数写入fp指向的c:\a.txt文件
		fprintf(fp, "%d\n", eNB->common_vars.txdata[eNB_id][0][i]);
	}
	fclose(fp);               //写入完毕，关闭文件*/

frame_parms->samples_per_tti = frame_parms->samples_per_tti*2;
frame_parms->ofdm_symbol_size = frame_parms->ofdm_symbol_size*2;
/*--------------------------------------END-------------------------------------------*/
	    ...
	    */
            ...
            do_OFDM_mod_symbol(&eNB->common_vars,
			       ...
			       eNB->do_precoding);
//加的
 write_output("txsig2.m","txs2", &eNB->common_vars.txdata[eNB_id][0][subframe*eNB->frame_parms.samples_per_tti], eNB->frame_parms.samples_per_tti,1,1);
            write_output("txsigF2.m","txsF2", &eNB->common_vars.txdataF[eNB_id][0][subframe*nsymb*eNB->frame_parms.ofdm_symbol_size],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);
	    ...
	    do_OFDM_mod_l(eNB->common_vars.txdataF[eNB_id],
			  ...
			  &eNB->frame_parms);
//加的
write_output("txsig3.m","txs3", &eNB->common_vars.txdata[eNB_id][0][subframe*eNB->frame_parms.samples_per_tti], eNB->frame_parms.samples_per_tti,1,1);
 write_output("txsigF3.m","txsF3", &eNB->common_vars.txdataF[eNB_id][0][subframe*nsymb*eNB->frame_parms.ofdm_symbol_size],nsymb*eNB->frame_parms.ofdm_symbol_size,1,1);
...
//加
            write_output("rxsig1.m","rxs1", &UE->common_vars.rxdata[0][0],10*UE->frame_parms.samples_per_tti,1,1);
	    write_output("rxsigF1.m","rxsF1", &UE->common_vars.common_vars_rx_data_per_thread[UE->current_thread_id[subframe]].rxdataF[0][0],UE->frame_parms.ofdm_symbol_size*nsymb,1,1);
	  ...
	  slot_fep(UE,
		   ...
		   0);
//加
            write_output("rxsig2.m","rxs2", &UE->common_vars.rxdata[0][0],10*UE->frame_parms.samples_per_tti,1,1);
	    write_output("rxsigF2.m","rxsF2", &UE->common_vars.common_vars_rx_data_per_thread[UE->current_thread_id[subframe]].rxdataF[0][0],UE->frame_parms.ofdm_symbol_size*nsymb,1,1);
...
//加
            write_output("rxsig3.m","rxs3", &UE->common_vars.rxdata[0][0],10*UE->frame_parms.samples_per_tti,1,1);

	    write_output("rxsigF3.m","rxsF3", &UE->common_vars.common_vars_rx_data_per_thread[UE->current_thread_id[subframe]].rxdataF[0][0],UE->frame_parms.ofdm_symbol_size*nsymb,1,1);

	  phy_procedures_UE_RX(UE,proc,0,0,dci_flag,normal_txrx,no_relay,NULL);

//加
 write_output("rxsig4.m","rxs4", &UE->common_vars.rxdata[0][0],10*UE->frame_parms.samples_per_tti,1,1);

	    write_output("rxsigF4.m","rxsF4", &UE->common_vars.common_vars_rx_data_per_thread[UE->current_thread_id[subframe]].rxdataF[0][0],UE->frame_parms.ofdm_symbol_size*nsymb,1,1);
